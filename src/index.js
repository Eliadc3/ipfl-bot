  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (
        ...
        return await handleGoogleOAuthStart_(url, env);
      }
      ...
      if (
        ...
        return await handleGoogleOAuthCallback_(url, env);
      }
      ...
      if (
        request.method === 'GET' &&
        url.pathname === '/api/google/status'
      ) {
        return await handleGoogleStatus_(url, env);
      }
      ...
      if (
        ...
        return await handleCalendarFeed_(url);
      }
      ...
      if (request.method === 'POST') {
        return await handleTelegramWebhook_(request);
      }
      ...
      // בזמן ההקמה עדיף שנראה את השגיאה בפועל
      return new Response(
        'Worker error: ' +
          (error && error.message
            ? error.message
...
async function handleGoogleOAuthStart_(url, env) {
  ...
  if (!/^\d+$/.test(chatId)) {
    return new Response(
      'Missing or invalid Telegram chatId.',
      { status: 400 }
    );
  }

  const state = await createSignedState_(
    {
      chatId,
      ts: Date.now()
    },
...
async function handleGoogleOAuthCallback_(url, env) {
  ...
  if (!code || !state) {
    return new Response(
      'OAuth callback is missing code/state.',
      { status: 400 }
    );
  }

  const stateData =
    await verifySignedState_(
      state,
      env.OAUTH_STATE_SECRET
    );
  ...
  const ageMs =
    Date.now() - Number(stateData.ts || 0);
  ...
  const chatId =
    String(stateData.chatId || '').trim();

  if (!/^\d+$/.test(chatId)) {
    return new Response(
      'Invalid Telegram chatId in OAuth state.',
      { status: 400 }
    );
  }

  const tokens =
    await exchangeCodeForTokens_(code, env);
  ...
  // אם כבר קיים חיבור, נשמור את ה-refresh token הישן
  // במקרה ש-Google לא מחזירה חדש.
  const existing =
    await env.DB
      .prepare(
        `SELECT
  ...
  let calendarId =
    existing && existing.calendar_id
      ? String(existing.calendar_id)
      : '';

  // אם עדיין אין יומן אישי - יוצרים אחד.
  if (!calendarId) {
    const calendar =
      await createGoogleCalendar_(
        accessToken
      );
    ...
    if (!calendarId) {
      throw new Error(
        'Google Calendar API did not return calendar id.'
      );
    }
  }
    ...
    await fetch(
      GOOGLE_CALENDAR_API + '/calendars',
      {
        ...
        body: JSON.stringify({
          ...
          description:
            'יומן משחקים אישי המתעדכן אוטומטית על ידי IPFL Bot.',
...
async function handleCalendarFeed_(url) {
  const token =
    decodeURIComponent(
...
async function handleTelegramWebhook_(request) {
  try {
    const body =
      await request.text();
    ...
    // Telegram לא צריך לנסות שוב שוב במקרה של תקלה זמנית.
    return new Response(
      'OK',
      { status: 200 }
    );
